...
def get_structures(apiKey, elements):
    ...
    "Pass in api key, a list of elements in the sample, and the number of different elements in the sample"
    mpr = MPRester(apiKey)
...
def read_data(fileName, startCut):
    "Reads in the data from the sample chi file and makes an initial cut at the beginning of the data"
    "Used to cut out air scatter and the beam stop from the start of the data"
...
def plot_background_and_structure(models, modelIndx, qcut, iqcut, plot_this=False):
    """Cuts the sample data to match where a single model's peaks are, then finds and removes the background.
    If plot_this is set to True, plots the cut data and the calculated background,
    then the sample data with no background and the position of the model peaks"""
    ...
    for i in range(len(model_num)):
        ...
        if x_val_mod[i]>=qcut[0]: #choosing the peaks to use based on where the data starts
            effective_peak_x.append(x_val_mod[i])
            effective_peak_y.append(y_val_mod[i])

    qcutM, iqcutM = cut_data(qcut, iqcut, 0, effective_peak_x[-1]+.1) #cut the data again for each model
    ...
    if plot_this:
        ...
        plt.scatter(effective_peak_x, effective_peak_y, label = "Model", marker = '.', color = 'r')
        plt.legend(loc=0)
        
def identify_phase(models, qcut, iqcut):
    """Identifies the most likely spacegroup symbol and material_id based on the sample data and Materials Project models.
    Pass in the list of models, qcut, and iqcut.
    This function attempts to fit a curve to the data based on the position and amplitudes of the models.
    It ignores models that look like they have too many peaks and it looks for the model with the
    ...
    for j in range(len(models)):
        ...
        peaks, _ = find_peaks(iqcutNoBgd, height=10,prominence=1) #find the number of sample data peaks
        ...
        for i in range(len(effective_peak_x)):
            if effective_peak_x[i]>=qcut[2]:
                ...
                y = effective_peak_y[i]
                peak_max_indx = 2 #used to see if there's a better peak somewhere else
                indx = find_nearest(qcut, x)

                my_max = np.max(iqcutNoBgd[indx-peak_max_indx:indx+peak_max_indx])
        ...
        if len(effective_peak_x) <= 2*len(qcutM[peaks]): #cuts out models with way too many peaks
            plt.figure()
            plt.plot(qcutM, iqcutNoBgd)
            plt.scatter(effective_peak_x, effective_peak_y, label = "Model", marker = '.', color = 'r')
            plt.plot(qcutM, sum_gauss(qcutM, *initial_guess), label="Initial Guess")
            plt.title(models[j]['material_id'] + "\t" + models[j]['spacegroup']['symbol'])

            print("zero count is " + str(zero_count))

            try:
                fit_data, _ = curve_fit(sum_gauss, qcutM, iqcutNoBgd, p0 = initial_guess, bounds = ([low_lims, high_lims]), maxfev=100)
            ...
            print("I think curve fit will fail for " + models[j]['material_id'] + ". There might be too many peaks")

    #find index of smallest residual
    smallIndx = resSumList.index(min(resSumList))
    print("I think you're looking at " + models[modelList[smallIndx]]['spacegroup']['symbol'] + 
